...
packets = rdpcap('nab_filtered.pcap')
...
def main():
    ...
    countByLength()
    dropOutliersBelowCount = int(input('Drop outliers below what count? '))
    counts = calcStdDeviation(countByLength(), dropOutliersBelowCount)
    create_schema_for = int(input('Which size packet would you like to investigate? '))
    threshold_count = int(input('What threshold count? '))
